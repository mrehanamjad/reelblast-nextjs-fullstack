...
import { Home, Search, PlusSquare, Menu, X, Clapperboard, Film, LogOut, LogIn, UserPlus } from "lucide-react"
import Link from "next/link"
import { useSession } from "next-auth/react"
import ProfilePic from "./UserComps/ProfilePic"
import { useUserProfile } from "./UserProfileContext"
...
export const NavItem: React.FC<NavItemProps> = ({ icon, label, active, onClick, href }) => {
  return (
    <Link href={href}>
      <button
        onClick={onClick}
...
export const MobileNavItem: React.FC<NavItemProps> = ({ icon, label, active, onClick, href }) => {
  return (
    <Link href={href} className="flex-1">
      <button
        onClick={onClick}
        className={`flex flex-col items-center justify-center w-full py-2 px-1 transition-colors ${
          active ? "text-blue-600 dark:text-blue-300" : "text-gray-700 dark:text-gray-100"
        }`}
      >
...
export default function SideNavbar() {
  ...
  const navItemsData = [
    ...
    {
      icon: <Clapperboard size={20} />,
      mobileIcon: <Clapperboard size={22} />,
      label: "Followings",
      href: "/followings",
      showOnMobile: false,
      isLogedIn: true,
    ...
    {
      icon: <ProfilePic size="sm" name={user?.name ?? ""} url={user?.profilePic.url ?? ""} />,
      mobileIcon: <ProfilePic size="sm" name={user?.name ?? ""} url={user?.profilePic.url ?? ""} />,
      label: "Profile",
      href: `/${session?.user.username ?? ""}`,
      showOnMobile: true,
      isLogedIn: true,
  ...
  return (
    <>
      ...
      {/* Mobile Bottom Navigation */}
      <div className="sm:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-black/95 backdrop-blur-md border-t border-gray-200/50 shadow-lg">
        <div className="flex items-center justify-between px-2 py-2 safe-area-inset-bottom">
          {mobileNavItems.map((nav) => (
            <MobileNavItem
              key={nav.label}
              icon={nav.mobileIcon || nav.icon}
      ...
      {/* Mobile Bottom Space - Removed as we're handling this in VideoComponent */}
    </>
  )
}
