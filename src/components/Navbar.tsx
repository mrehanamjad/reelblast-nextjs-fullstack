...
export default function Navbar() {
  ...
  useEffect(() => {
    if (pathname === "/") {
      setActiveItem("home"); 
    } else if (pathname === "/video/upload") {
      setActiveItem("create");
    } else if (pathname === "/search") {
      setActiveItem("search");
    } else if (pathname === "/followings") {
      setActiveItem("followings");
    } else if (pathname === "/register") {
      setActiveItem("signup");
    } else if (pathname === "/login") {
      setActiveItem("login");
    } else if (pathname === `/${session?.user.username}`) {
          ...
          <div className="space-y-2">
            {filteredNavItems.map((nav) => (
              <NavItem
                ...
                active={activeItem === nav.key}
                onClick={() => handleItemClick(nav.key)}
                href={nav.href}
              />
            ))}
            ...
            <div className="border-t  border-gray-200/10  py-3 my-2 w-11/12 absolute bottom-0">
              ...
              <div
                className={`flex border-t-amber-900 items-end justify-start gap-5 ${
                  collapsed ? "px-2" : "px-4"
                } py-1 h-9 mt-5 shadow-md max-sm:hidden`}
              >
                <Link
                ...
                <Link href="https://github.com/mrehanamjad" target="_blank">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 496 512"
                    fill="white"
