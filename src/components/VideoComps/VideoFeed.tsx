...
export default function VideoFeed({ videos, playingVideoIndex = 0 }: VideoFeedProps) {
    ...
    if (!isLoading && videos.length > 0) {
      ...
      observerRef.current = new IntersectionObserver(
        (entries) => {
          ...
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              const index = Number.parseInt(entry.target.id.split("-")[1])
              setCurrentVideoIndex(index)
            }
          })
        },
  ...
  return (
    <div
      ref={containerRef}
      className="bg-black min-h-screen relative w-full flex justify-center"
      onTouchStart={handleTouchStart}
      onTouchEnd={handleTouchEnd}
    >
      <ReelNavMobile />
      {isLoading ? (
        <div className="h-screen w-full flex justify-center items-center">
          <div className="w-12 h-12 sm:w-16 sm:h-16 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin"></div>
        </div>
      ) : videos.length > 0 ? (
        <div className="w-full max-w-md mx-auto relative">
          <div ref={feedRef} className="h-screen snap-y snap-mandatory overflow-y-scroll scrollbar-hide">
            {videos.map((video, index) => (
              <div
                key={video._id?.toString() || index}
                id={`reel-${index}`}
          ...
          {!isLoading && videos.length > 1 && (
            <>
              <button
                onClick={goToPrevVideo}
                disabled={currentVideoIndex === 0}
                className={`fixed z-30 right-2 sm:right-4 top-1/4 transform -translate-y-1/2 bg-black/50 backdrop-blur-sm rounded-full p-2 sm:p-3 ${
                  currentVideoIndex === 0
              ...
              <button
                onClick={goToNextVideo}
                disabled={currentVideoIndex === videos.length - 1}
                className={`fixed z-30 right-2 sm:right-4 top-1/2 transform -translate-y-1/2 bg-black/50 backdrop-blur-sm rounded-full p-2 sm:p-3 ${
                  currentVideoIndex === videos.length - 1
                    ? "opacity-20 cursor-not-allowed"
                    : "opacity-60 hover:opacity-90 active:scale-95"
                } transition-all duration-200`}
        ...
        <div className="h-screen flex flex-col items-center justify-center text-white p-4 max-w-md mx-auto pt-16">
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-indigo-900 flex items-center justify-center mb-4">
            <span className="text-2xl sm:text-3xl">🎬</span>
          </div>
          <p className="text-lg sm:text-xl font-medium mb-2 text-center">No reels found</p>
          <p className="text-gray-400 text-center text-sm sm:text-base">Discover trending videos or create your own</p>
          <button className="mt-6 px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-full font-medium text-sm sm:text-lg hover:scale-105 active:scale-95 transition-transform">
            Explore Reels
          </button>
        </div>
      )}
